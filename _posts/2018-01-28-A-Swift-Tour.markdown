...
用三双引号 """ 来定义多行的字符串，每行字符的缩进都和结束的三双引号的缩进相同，如下所示（官方文档此处举例比较简单，举例及注释为个人理解所加，与原版有出入）：
...
occupations = [:]
控制流程
if 和 switch 用来做条件判断，for - in ， while 和 repeat - while 用来做循环。判断条件和循环变量的圆括号可以省略，但是语句体的大括号不能：
